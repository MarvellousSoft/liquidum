...
func display_day(data: RecurringMarathon.LeaderboardData, date: String) -> void:
	...
	for item in data.list:
		...
		if item.flair != null:
			flair.text = "  %s  " % [item.flair.text]
			flair.add_theme_color_override("font_color", item.flair.color)
			var contrast_color := Global.get_contrast_background(item.flair.color)
