...
test.describe("Leaderboard & PlayFab E2E Tests", () => {
  ...
  test("submits daily score on first victory and removes Play Again button", async ({
    ...
    // Leaderboard modal opened automatically upon score submission
    await expect(page.locator('[data-testid="leaderboard-modal"]')).toBeVisible();
    await page.click('button[aria-label="Close"]');
    await expect(page.locator('[data-testid="leaderboard-modal"]')).toHaveCount(0);

    // Verify Leaderboard button on win banner can reopen it
    ...
    await page.evaluate(() => (window as any).restartLevel());
    // Solve level again
    await page.evaluate(() => {
      const g = (window as any).getGridData();
      if (!g || !g.solution_c_left) return;
  ...
  test("removes leaderboard button while on test levels", async ({ page }) => {
    await page.goto("/?level=01/01");
    await expect(page.locator('[data-testid="cell-0-0"]')).toBeVisible();

    // Leaderboard button in toolbar should NOT exist
    await expect(page.locator('[data-testid="btn-leaderboard"]')).toHaveCount(0);
    // Desktop side panel leaderboard should NOT exist
    await expect(page.locator('[data-testid="desktop-leaderboard"]')).toHaveCount(0);
  ...
  test("automatically shows leaderboard modal after daily level completion and score submission", async ({ page }) => {
    let statsUpdateCalled = false;
    ...
    // Score is submitted to PlayFab
    await expect.poll(() => statsUpdateCalled).toBe(true);

    // Leaderboard modal should automatically appear after submission
    await expect(page.locator('[data-testid="leaderboard-modal"]')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('[data-testid="leaderboard-modal"]')).toContainText("Daily Leaderboard");
  });
});
