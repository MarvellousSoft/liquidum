...
export function LeaderboardModal({ isOpen, onClose }: LeaderboardModalProps) {
  ...
  return (
    <div className="modal-backdrop fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div
        className="modal-content bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col shadow-2xl overflow-hidden text-slate-100"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 id="leaderboard-title" className="text-xl font-bold text-cyan-400 leading-tight">
                Daily Leaderboard
              </h2>
          ...
          <button
            className="p-1 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition text-lg leading-none w-8 h-8 flex items-center justify-center"
            onClick={onClose}
            aria-label="Close"
          >
            ✕
        ...
        {/* User Profile Bar */}
        <div className="px-6 py-3 bg-slate-800/50 border-b border-slate-800/80 flex items-center justify-between text-xs">
          {isEditingName ? (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="text-slate-400 border-b border-slate-800">
                  ...
                  <th className="py-2 px-3 text-center w-20">Mistakes</th>
                  <th className="py-2 px-3 text-right w-20">Time</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry) => {
                  ...
                  return (
                    ...
                    >
                      <td className="py-2 px-2 text-center">
                        {medal ? <span className="text-base">{medal}</span> : entry.position}
                      </td>
